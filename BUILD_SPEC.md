...
**미션(대상·목표) — SSOT:** 아무것도 모르는 **취업 초보(취린이)부터 노련한 경력자까지** 누구나 쉽게 **채용 공고에 지원**하도록 돕는다. 나아가 사용자가 **시장에서 자기 위치**를 알고, 그에 따라 **무엇을 준비**할지, **PR(자기표현)은 어떻게** 할지 알게 한다. → 대상 스펙트럼 전체를 위해 **초보 진입 스캐폴딩**(콜드스타트 오리엔테이션·전문용어 즉시 풀이·easy 기본 렌더·"빠른 첫 결과")과 **경력자 저마찰 패스트레인**(문서→뱅크 자동 인제스트·verdict-first)을 **동시에** 제공하되, 단일 목적·점진 공개·상주 UX 금지 원칙은 유지한다.
...
7. **커리어 로드맵** — 3/5/10년, 현 진단(+시장 위치 레이어) → 다중 경로(직무)·직급 라더 → 목표 → 스킬 → 증거 → 실행

> **미션 완결 오버레이(SKILL §6.1):** 7개 핵심 태스크에 **상황 감지 시** 얹는 조건부 확장 — 전형 관문(인적성·필기·**코딩테스트**)·직무 탐색 콜드스타트(제로베이스)·공백기 재취업·네트워킹/리퍼럴·**지원 제출 실행**·해외취업 비자/스폰서십·오퍼 후 이직 실행(레퍼런스·근로계약)·탈락 대응 루프. **새 커맨드가 아니라 오버레이/체이닝**(D-9 단일 목적 유지), 산출은 기존 템플릿 재사용, "자문 아님" 고지.

...
> 💬 **취업·이직을 도와주는 도구예요. 아무것도 몰라도 괜찮습니다** — `/career` 치고 **"취업 준비 도와줘"**라고
> 말만 하면 무엇부터 할지 안내해 드립니다. **이력서 없어도 OK**, 어려운 말은 그때그때 풀어드려요.
> (설치가 어렵게 느껴지면 아래 *설치*는 건너뛰고 *사용법*만 봐도 됩니다.)
...
> 🚀 **처음이신가요? 이거면 됩니다:** `/career` 치고 **"취업 준비 도와줘"**(또는 하고 싶은 말)라고 하세요.
> 스킬이 **무엇을 도울 수 있는지 안내하고, 어디서 시작할지 추천**합니다. **이력서·경력이 없어도 됩니다** — 대화로
> 하나씩 채웁니다. **전문용어는 처음 나올 때 풀어서** 설명하고, 결과는 **쉬운 말 버전**으로 먼저 보여줍니다.
> 급하면 지원할 **공고 하나**나 **자소서 문항 하나**만 가져오세요 — 그것만으로 바로 시작합니다.
...
2. **말하기:** 하고 싶은 걸 자연어로. 스킬이 태스크를 자동 판별한다(메뉴 없음).
   ...
   - "오퍼 받았는데 연봉협상" → ⑥ 연봉협상
   - "3/5/10년 로드맵" → ⑦ 커리어 로드맵
3. **개인 자료 (있으면 좋고, 없어도 됨):** 이력서·포트폴리오가 있으면 주면 스킬이 알아서 정리해 씁니다.
   없으면 대화로 채웁니다. **개인정보는 이 기기의 비공개 폴더(`.private/`)에만 저장됩니다** — 스킬이 저장 전
   `.gitignore` 등록까지 확인하고 알려줍니다(커밋 방지). **외부로 전송되지 않습니다** — 단, 사용자가
   *직접* 개인 허브에 자기 백엔드/AI를 연결한 경우에만 그 사용자 소유 서버로 전송됩니다(선택 기능).
4. **평가 강도:** 코칭 / 균형(기본) / 압박 중 실행 중 선택.
5. **산출물:** 분석·리포트는 HTML 보고서, 문서(이력서 등)는 A4 인쇄용 HTML. 브라우저에서 열고
   인쇄→PDF로 A4에 깔끔히 앉는다.

## 포트폴리오 빌더 = 기초 job (P0–P8) <sub>— 고급·선택, 처음엔 안 봐도 됩니다</sub>
이 스킬의 토대는 **마스터 경험 뱅크**(`.private/experience-bank.md`) — 모든 경험을 한 번 정리해 두면
지원마다 재입력 없이 맞춤 문서를 파생한다.

> ⚡ **급하면 다 안 채워도 됩니다.** 지금 지원할 **공고 하나**나 **자소서 문항 하나**만 가져오세요 —
> 스킬이 **필요한 것만 그때그때** 묻고, 경험 뱅크는 쓰면서 조금씩 채웁니다. 아래 P0–P8은 "한 번
> 완주하면 두고두고 편한" 철저한 경로일 뿐 **필수 첫 단계가 아닙니다.**
> 🧭 **처음이라 뭘 할지 모르겠으면** 그냥 "취업 준비 도와줘"라고 하세요 — 무엇을 도울 수 있는지
> 안내하고, ⑤ 자가진단이나 경험 정리부터 권합니다.
...
> **완결성 오버레이(§6.1):** 전형 관문(인적성·필기·코딩테스트)·직무 탐색 콜드스타트·공백기 재취업·네트워킹/리퍼럴·지원 제출 실행·해외취업 비자·오퍼 후 이직 실행·탈락 대응 루프는 **상황 감지 시** 기존 태스크에 얹힌다(새 메뉴·커맨드 아님). "자문 아님" 고지 상속.
